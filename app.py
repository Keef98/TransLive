...
def filter_non_english(text):
    ...
    filtered_words = [word for word in words_list if word.lower() in ENGLISH_WORDS or word.istitle()]
...
def start_audio_stream():
    ...
    with sd.InputStream(device=INPUT_DEVICE, samplerate=44100, channels=CHANNELS, dtype='int16',
                        callback=audio_callback, blocksize=44100):
        audio_buffer = []
        silence_time = 0
...
def recognize_and_translate(audio_chunk):
    try:
        ...
        if text.strip():
            ...
            threading.Thread(target=text_to_speech_offline, args=(translated_text,)).start()
